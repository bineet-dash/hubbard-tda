...
int main(int argc, char* argv[])
{
  ...
  double free_energy = spa_free_energy(H_spa, final_temp, sigma);
  ...
  filename="data/m_length_spa_"+ current_time_str()+latticedata+".txt"; ofstream outfile_mlength(filename);
  filename="data/spa_results_"+current_time_str()+latticedata+".txt"; ofstream outfile_freeenergy(filename);
    ...
    for(double i=10; i>=2; i-=1)
    {
      ...
      for(int sweep=0; sweep<N_therm; sweep++)
      {
        for(int lattice_index=0; lattice_index<L; lattice_index++)
        {
          ...
          double suggested_free_energy = spa_free_energy(suggested_Hspa,temperature,suggested_sigma);
